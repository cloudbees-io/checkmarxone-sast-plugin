...
NOTE: For more information about the Checkmarx One SAST scanner, refer to the link:https://docs.checkmarx.com/en/34965-68517-checkmarx-one-user-guide.html[Checkmarx One documentation].
...
The following CloudBees platform workflow example scans code and filters results to show only critical and high severity findings.
    ...
    steps:
      ...
      - id: print-outputs-from-checkmarxone-sast-step
        ...
        run: |
            ...
            echo "Policy subject: ${{steps.checkmarxone-sast-step.outputs.policy-subject}}"
...
jobs:
  job1:
    outputs:
      ...
      checkmarxone-sast-job-output-policy-subject: ${{ steps.checkmarxone-sast-step.outputs.policy-subject }}
    steps:
      - name: check out source code
        uses: cloudbees-io/checkout@v1
        with:
    ...
    steps:
      - id: print-outputs-from-job1
        ...
        run: |
          ...
          echo "Policy subject: ${{ needs.job1.outputs.checkmarxone-sast-job-output-policy-subject }}"
...
* Learn about link:https://docs.checkmarx.com/en/34965-322308-checkmarx-one-scanners.html[Checkmarx One SAST].